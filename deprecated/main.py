...
class MemBlocksCLI:
    ...
    def display_status(self):
        ...
        if self.current_session_id:
            ...
            attached_block_id = (
                session_info.get("attached_block_id") if session_info else None
            )
            ...
            if self.chat_service:
                ...
                print(
                    f"   📝 Summary: {'Yes' if self.chat_service.recursive_summary else 'No'}"
                )
        else:
    ...
    async def select_create_user(self):
        ...
        choice = (
            await self.ainput("\nEnter user ID (or press Enter for current): ")
        ).strip()
    ...
    async def start_chat_session(self):
        ...
        try:
            ...
            session_manager.attach_block(
                self.current_session_id, selected_block.meta_data.id
            )

            # Initialize chat service
            self.chat_service = ChatService(
                memory_block=selected_block, memory_window=10, keep_last_n=4
            )
