...
\begin{enumerate}
    ...
    \item To organize each memory block into structured sections, including core memory (essential facts), semantic memory (knowledge and events), episodic memory (conversation summaries), and resources (documents).
...
This chapter presents the comprehensive design and implementation methodology for the MemBlocks system. The architecture addresses fundamental limitations in current Large Language Model (LLM) memory systems by introducing a modular, hierarchical approach to context management. Unlike traditional Retrieval-Augmented Generation (RAG) systems that treat all information uniformly in a single vector database, or systems like MemGPT and Mem0 that provide limited memory differentiation, MemBlocks uses a library-first implementation centered on MemBlocksClient with distinct memory containers, section-specific retrieval, and structured update workflows. This section details the system architecture, algorithmic workflows, data structures, and implementation technologies that collectively enable composable, context-aware memory management for AI agents.

\subsection{Feasibility Study}

The MemBlocks project is technically feasible due to recent advancements in Large Language Models, vector databases, and practical integration tooling. The system builds upon reliable and well-documented technologies such as Groq-hosted LLM APIs for inference, Qdrant for high-performance vector storage, Ollama for local embedding generation, and a Python client library interface (MemBlocksClient) with optional MCP integration for agent-facing workflows. These tools provide the necessary capabilities for implementing modular memory containers, semantic retrieval, and metadata-driven memory management without requiring the development of core ML models from scratch.
...
\hline
Sectioned Memory Structure & Each memory block must internally support structured sections such as core memory, semantic memory, episodic memory, and resources. \\
\hline
...
Memory Block Repositories serve as independent storage units, each maintaining its own organizational structure and access semantics. Unlike monolithic RAG systems, where documents, chat history, and user preferences co-exist in a single vector database, MemBlocks segregates information into functionally distinct containers. Each container encapsulates multiple sections, including Resources (immutable user documents and chat transcripts), Core Memory (persistent persona and user attributes), Semantic Memory, and Episodic Memory (conversation summaries). This segregation reduces retrieval noise and enables fine-grained access control.


The retrieval layer serves as the bridge between long-term memory blocks and the agent's active context, ensuring that only relevant information is provided to the LLM. It operates exclusively on attached memory blocks, accessing their structured sections: core memory, semantic memory, episodic memory, and resources using a hybrid approach that combines semantic similarity over vector embeddings with metadata-based filtering such as recency, confidence, visibility, and type. Core memory and episodic summaries are always included, while semantic memories are ranked using a composite scoring strategy. Finally, the processed context is merged with the sliding window of recent conversation turns, producing a compact, contextually rich input for the answering LLM. This design maintains modularity, reduces retrieval noise, enforces memory isolation across blocks and sections, and ensures low-latency, accurate context assembly for effective agent reasoning.
